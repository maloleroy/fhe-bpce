...
use seal_lib::{CkksHOperation2, SealCkksCS, context::SealCkksContext};
use std::sync::Mutex;

const BATCH_SIZE: usize = 1 << 15; // 32 KB

fn main() {
    let bfv_ctx = SealCkksContext::new(seal_lib::DegreeType::D4096, seal_lib::SecurityLevel::TC128);
    let bfv_cs = SealCkksCS::new(&bfv_ctx, 307727.0);
    ...
    let sum = Mutex::new(bfv_cs.cipher(&0.0));
    ...
    iter.par_bridge().for_each(|batch| {
        ...
        let mut local_sum = bfv_cs.cipher(&0.0);

        for i in 0..array.len() {
            let v_cipher = bfv_cs.cipher(&array.value(i));
            bfv_cs.operate2_inplace(CkksHOperation2::Add, &mut local_sum, &v_cipher);
        }

        println!("Summed {} f64 of the column \"rwa\".", BATCH_SIZE);

        let mut sum_lock = sum.lock().unwrap();
        bfv_cs.operate2_inplace(CkksHOperation2::Add, &mut sum_lock, &local_sum);
    });
